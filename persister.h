...
//     if(file_size + put_basic_length > MAX_LOG_SZ) {
//         out_file.close();
//         checkpoint();
//         std::ofstream tmp_file(file_path_logfile, std::ios::out | std::ios::binary | std::ios::trunc);
//         out_file = std::move(tmp_file);
//     }
...
//         // need extra judge
//         if(file_size + size + put_basic_length > MAX_LOG_SZ) {
//             out_file.close();
//             checkpoint();
//             std::ofstream tmp_file(file_path_logfile, std::ios::out | std::ios::binary | std::ios::trunc);
//             out_file = std::move(tmp_file);
...
//     // first, find whether there are some unfinished transaction in the tail, if has, reput to log
//     // int j = n - 1;
//     // std::vector<command> tails;
//     // while(j >= 0 && logs[j].type != chfs_command::cmd_type::CMD_COMMIT) {
//     //     tails.emplace_back(logs[j]);
//     //     --j;
//     // }
//     // if(j < 0) {
//     //     printf("something wrong in checkpoint!\n");
//     // } 


//     // second, find all entry don't need to store
//     // for put, only save the last write
//     // n = j;
