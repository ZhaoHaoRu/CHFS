...
unit_test_case *unit_test_suite::register_test_case(const char *part,
                                                    const char *name,
                                                    unit_test_case *obj) {
...
bool unit_test_suite::run_case(const std::string &part, const std::string &name,
                               unit_test_case *obj) {
  if (is_counting) {
    count++;
    return true;
  ...
  std::cout << "Pass (" << part << "." << name
            ...
            << "s, user-time: "
            << (double)(end_t.tms_utime - start_t.tms_utime) / (double)clktck
            << "s, sys-time: "
            << (double)(end_t.tms_stime - start_t.tms_stime) / (double)clktck
            << "s" << std::endl;
  passed++;
  return true;
}

bool unit_test_suite::run_part_case(const std::string &spart,
...
list_state_machine::list_state_machine() {
  store.push_back(0);
  num_append_logs = 0;
  // Because the log is start from 1, so we push back a value to align with the
  // raft log.
}
